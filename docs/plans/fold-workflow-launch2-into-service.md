...
| Launcher `.github/.labels.json` = 32 labels, and is exactly the dispatch vocabulary `filters.py` / `prompt_builder.py` match (`orchestration:*`, `implementation:ready\|complete`, `gh-issue-tracking:direct-body`, `agent:*`, `state:*`) | label-name set vs. `grep` of `src/webhook_receiver` |
