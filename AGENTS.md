...
- **Scripts**: [`.agents/rules/scripts.md`](.agents/rules/scripts.md) — repo-root `scripts/` inventory (auth helpers, `import-labels.ps1` for label sync, `query.ps1` as the canonical PR review-thread manager, dispatch-issue creator, permission verifier, remote-index regenerator) and the new-app launch pipeline, whose entry point `create-repo-agent-context.ps1` seeds a template clone from the `plan_docs/` slug store still held in the sibling `workflow-launch2` checkout. Read each script's header + `param()` block for authoritative docs.
- **Repository & Clone Lifecycle**: [`.agents/rules/lifecycle.md`](.agents/rules/lifecycle.md) — the approved map of who seeds whom: **this repo is the lifecycle owner** (launcher, receiver, ACP host, SwarmSandbox solution, `$swarm` system; `isTemplate: false`), `nam20485/swarm-context` is **the parent template / clone stamper** (pruned to "agent-context base + swarm surfaces"; clones stamped from it carry `.zcode/` + swarm rules + `$swarm` skills), `intel-agency/agent-context` is the parent's upstream base (the stamper **before** the 2026-09-23 flip — why every pre-flip `gap-miner-v2-*` clone has no `.zcode/`), and `workflow-launch2` is the plan-docs slug store (with stale launcher copies — never run from there). Includes the envelope path, the ACP-client parity rule (clients additive + config-mirrored — opencode and ZCode both, the sandbox runs ZCode), and the agent-definition directory map.
